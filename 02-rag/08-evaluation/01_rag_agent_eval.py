...
def retrieve_context(query: str, config: RunnableConfig):
    ...
    # as_retriever() + invoke() garante que os callbacks do LangGraph/Langfuse (via config) 
    # desçam para o modelo de embeddings que roda dentro da vectorstore
    retriever = vectorstore.as_retriever(search_kwargs={"k": 2})
    retrieved_docs = retriever.invoke(query, config=config)
...
def main():
    ...
    print(f"\nUser: {query1}")
    for event in agent_executor.stream({"messages": [("user", query1)]}, stream_mode="values", config=config):
        event["messages"][-1].pretty_print()

    # Exemplo 2: Chitchat
    query2 = "Oi, tudo bem?"
    print(f"\nUser: {query2}")
    for event in agent_executor.stream({"messages": [("user", query2)]}, stream_mode="values", config=config):
        event["messages"][-1].pretty_print()

    # Exemplo 3: Pergunta fora do contexto (Fallback)
    query3 = "Qual é a cotação atual do Bitcoin?"
    print(f"\nUser: {query3}")
    for event in agent_executor.stream({"messages": [("user", query3)]}, stream_mode="values", config=config):
        event["messages"][-1].pretty_print()
